...
const GroupChatDatabase = {
    ...
    getGroupsOfUser: async (idUser, page) => {
        ...
        const data = await GroupChat.find({'idUsers.idUser': {'$in': [idUser]}}).limit(perPage).skip(page * perPage).sort({lastSentTime: -1})
    ...
    getSpecificGroupChat: async (idUsers) => {
        try {
            return await GroupChat.findOne({'idUsers.idUser': {'$all': idUsers}}).lean()
        } catch (e) {
            return null
        }
    }
